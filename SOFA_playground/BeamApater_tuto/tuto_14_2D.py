...
class catheterController(Sofa.Core.Controller):
    ...
    def onKeypressedEvent(self,e):
        ConstantForce = self.instrument.CollectorForceField.forces[0][0:2]
        ...
        self.instrument.CollectorForceField.forces[0][:2] = [np.cos(theta)*amplitude, np.sin(theta)*amplitude]
...
def createScene(rootNode):
    ...
    BeamModel.addObject('ConstantForceField', name='CollectorForceField', indices=0, forces=[0,0,0,0,0,0],indexFromEnd=True, showArrowSize=5e1)
    BeamModel.addObject(catheterController(BeamModel))
