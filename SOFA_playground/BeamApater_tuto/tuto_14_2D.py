...
# Add Magnetic Field Torque Controller to catheter, illustrate the magnetic field.
# Author: JunAng Wang
...
class catheterController(Sofa.Core.Controller):
    ...
    def onAnimateBeginEvent(self, event):
        ...
        self.instrument.VisualCatheter.VisuOgl.OglLabel.label =f'{self.magnetic_field_spherical[0]} mT'
...
def createScene(rootNode):
    ...
    magnetic_field_spherical = [1,np.pi/2,np.pi/2]
    ...
    GuideCatheter.addObject('WireRestShape', name='GC_RestShape',  
                                 printLog=False, template='Rigid3d', wireMaterials = '@StraightSection_stiff @StraightSection_soft')
    ...
    BeamModel.addObject('ConstantForceField', name='CollectorMagneticForceField', indices=np.arange(nbsection_soft), forces=np.tile(np.zeros(6), (nbsection_soft,1)),indexFromEnd=True, showArrowSize=1e-2)
    ...
    BeamModel.addObject('InterventionalRadiologyController', name='DeployController', template='Rigid3d', instruments='BeamInterpolation', 
                                    ...
                                    rotationInstrument=[0, 0, 0], step=3, speed=4, 
