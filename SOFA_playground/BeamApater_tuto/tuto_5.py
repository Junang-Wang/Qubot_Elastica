...
def createScene(rootNode):
    ...
    # create straight Section, 5 sections, if want to import from external files using RodMeshSection
    GuideCatheter.addObject('RodStraightSection', name ='StraightSection',
                         youngModulus= 1e5, poissonRatio=0.3, radius = 0.1, massDensity=1, nbEdgesCollis = 5, nbEdgesVisu = 5, length = 3)
    ...
    VisualCatheter.addObject('Edge2QuadTopologicalMapping', nbPointsOnEachCircle=10,
                             radius=0.1, input='@../../GuideCatheter/meshLinesBeam', output='@visu_container')
    ...
    CollisionCatheter.addObject('Edge2QuadTopologicalMapping',nbPointsOnEachCircle=10,
                                radius=0.1, input='@../../GuideCatheter/meshLinesBeam',
                                output='@colli_container')

    # CylinderGridTopology, nz is the longitudinal direction discretization
    # CollisionCatheter.addObject('CylinderGridTopology', name ='Cylinder_container', axis= [1,0,0], center = [0,0,0], length=3, radius=0.1, nx=5, ny=5, nz=6)
