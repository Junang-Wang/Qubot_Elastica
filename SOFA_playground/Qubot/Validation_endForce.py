...
def rootNodeInit(forces, dt, youngModulus, length, radius):
    # Create the root node
    rootNode = Sofa.Core.Node("rootNode")
    rootNode.addObject('Camera',name='c')
    rootNode.c.position.value = [0,0,500]
    end_forces = [[0,forces,0,0,0,0]]
    createScene(rootNode, end_forces=end_forces, dt= dt, youngModulus = youngModulus, length = length, radius=radius)
...
def createScene(rootNode, dt=0.01, end_forces=[[0,10,0,0,0,0]], youngModulus = 1.4e6, length = 6, radius =0.3):
